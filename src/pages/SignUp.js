...
import { doestUsernameExists } from "../services/firebase";
...
function SignUp() {
  ...
  const handleSignUp = async (event) => {
    ...
    const usernameExists = await doestUsernameExists(username);
    ...
    <div className="container flex mx-auto max-w-sm items-center h-screen">
      <div className="flex flex-col mx-auto">
        <div className="flex flex-col items-center bg-white p-4 border border-gray-300 mb-4 rounded">
          {/* <h1 className="flex justify-center w-full">Photo Sharing</h1> */}
        ...
        <div className="flex justify-center items-center flex-col w-full bg-white p-4 rounded border border-gray-300">
          <p className="text-sm">
            Have an account?{" "}
            <Link to={ROUTES.LOGIN} className="font-bold text-bluish">
              Log In
            </Link>
          </p>
        </div>
